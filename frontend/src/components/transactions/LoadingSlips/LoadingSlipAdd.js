...
import { getLorryReceipts, addLoadingSlip } from '../../../lib/api-transactions';
import LoadingSpinner from '../../UI/LoadingSpinner';
import { mobileNoRegEx } from '../../../lib/helper';
import FreightDetails from './Freight-Details/FreightDetails';
...
const LoadingSlipAdd = () => {
  ...
  useEffect(() => {
    ...
    if (location.pathname) {
      location.pathname.endsWith('addLocalMemoLS') ? setIsLocalMemo(true) : setIsLocalMemo(false)
    }
  ...
  useEffect(() => {
    ...
    if (isSubmitted && !hasErrors) {
      ...
      addLoadingSlip(updatedLoadingSlip, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
  ...
  }, [isSubmitted, hasErrors, loadingSlip, goToLoadingSlips, isLocalMemo, goToLocalMemo]);
  ...
  useEffect(() => {
    const hireCost = (+loadingSlip.hire + +loadingSlip.hamali - +loadingSlip.commission - +loadingSlip.stacking - +loadingSlip.advance);
  ...
  return <>
    ...
    <h1 className='pageHead'>{isLocalMemo ? 'Add a local memo loading slip' : 'Add a loading slip'}</h1>
          ...
          <div className='grid-item'>
            <FormControl fullWidth size='small' error={formErrors.driver.invalid}>
              <Autocomplete
                ...
                openOnFocus
                renderInput={(params) => <TextField {...params} label="Driver" error={formErrors.vehicle.invalid} fullWidth />}
              />
    ...
    <Paper sx={{ padding: '20px', marginBottom: '20px' }}>
      <FreightDetails loadingSlip={loadingSlip} setLoadingSlip={setLoadingSlip} customers={customers} lorryReceipts={lorryReceipts} />
        ...
        <div className='grid grid-8-col'>
          <div className='grid-item'>
            <FormControl fullWidth error={formErrors.toPay.invalid}>
              <TextField size='small' variant='outlined' label='LR billing total' value={loadingSlip.toPay} error={formErrors.toPay.invalid} onChange={inputChangeHandler} name='toPay' id='toPay' inputProps={{ readOnly: true }} />
          ...
          <div className='grid-item'>
            <FormControl fullWidth error={formErrors.total.invalid}>
              <TextField size='small' variant='outlined' label='Total' value={loadingSlip.total} onChange={inputChangeHandler} name='total' id='total' inputProps={{ readOnly: true }} />

...
import { TextField, InputLabel, MenuItem, FormControl, FormHelperText, Button, Paper, Divider, InputAdornment } from '@mui/material';
...
import { getBill, getLorryReceiptsByConsignor, updateBill } from '../../../lib/api-transactions';
...
const BillEdit = () => {
  ...
  useEffect(() => {
    ...
    if (fetchedBill.branch && fetchedBill.customer) {
      setIsLoading(true);
      getLorryReceiptsByConsignor(fetchedBill.branch, fetchedBill.customer, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
            const updatedLR = [...response];
  ...
  }, [fetchedBill.branch, fetchedBill.customer, customers]);

  useEffect(() => {
    const controller = new AbortController();
    if (billId) {
      setIsLoading(true);
      getBill(billId, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
            setFetchedBill(response);
          }
          setIsLoading(false);
        })
        .catch(error => {
  ...
  useEffect(() => {
    if (fetchedBill._id && fetchedLorryReceipts.length && customers.length) {
      ...
      const updatedLorryReceipts = fetchedLorryReceipts.map(fetchedLR => {
        const isInBill = fetchedBill.lrList.filter(lr => lr._id === fetchedLR._id);
        return {
          ...fetchedLR,
      ...
      if (billId) {
        updatedFilteredLorryReceipts = updatedLorryReceipts.filter(lr => {
          return !lr.billGenerated || (lr.billGenerated && lr.assoBill === billId);
        });
      }

      updatedBill.lrList = updatedBill.lrList.map(billLr => {
        return updatedLorryReceipts.filter(lr => lr._id === billLr._id)[0];
      });
      setBill(updatedBill);
  ...
  const validateForm = (formData) => {
    ...
    if (!formData.totalAmount || isNaN(formData.totalAmount)) {
      errors.lrList = { invalid: true, message: 'Total amount should a number and greater than 0' };
    }
    if (formData.serviceTax && isNaN(formData.serviceTax)) {
      errors.serviceTax = { invalid: true, message: 'Service tax should be a number' };
    }
    if (!formData.dueDate) {
  ...
  return <>
    ...
    <form action='' onSubmit={submitHandler} id='billForm'>
      <Paper sx={{ padding: '20px', marginBottom: '20px' }}>
        <div className='grid grid-6-col'>
          ...
          <div className='grid-item'>
            <FormControl fullWidth size='small' error={formErrors.customer.invalid}>
              ...
              <Select
                ...
                onChange={inputChangeHandler}
              >
                {customers.length > 0 && customers.map(customer => <MenuItem key={customer._id} value={customer._id} className='menuItem'>{customer.name}</MenuItem>)}
              </Select>
              {formErrors.customer.invalid && <FormHelperText>{formErrors.customer.message}</FormHelperText>}
            </FormControl>
          </div>
        </div>
      </Paper>
    ...
    <Paper sx={{ padding: '20px', marginBottom: '20px' }}>
      ...
      <LorryReceipts lorryReceipts={lorryReceipts} setLRForBill={setLRForBill} bill={bill} setBill={setBill} />
        ...
        <div className='grid grid-6-col'>
          <div className='grid-item'>
            <FormControl fullWidth error={formErrors.totalAmount.invalid}>
              <TextField size='small' variant='outlined' label='Total amount' value={bill.totalAmount} error={formErrors.totalAmount.invalid} onChange={inputChangeHandler} name='totalAmount' id='totalAmount' InputProps={{
                startAdornment: <InputAdornment position='start'>&#8377;</InputAdornment>
              }} />
              {formErrors.totalAmount.invalid && <FormHelperText>{formErrors.totalAmount.message}</FormHelperText>}
            </FormControl>
          </div>
          <div className='grid-item'>
            <FormControl fullWidth error={formErrors.serviceTax.invalid}>
              <TextField size='small' variant='outlined' label='Service tax' value={bill.serviceTax | 0} error={formErrors.serviceTax.invalid} onChange={inputChangeHandler} name='serviceTax' id='serviceTax' InputProps={{
                endAdornment: <InputAdornment position='end'>%</InputAdornment>
              }} />
              {formErrors.serviceTax.invalid && <FormHelperText>{formErrors.serviceTax.message}</FormHelperText>}
            </FormControl>
          </div>
          <div className='grid-item'>
            <FormControl fullWidth error={formErrors.total.invalid}>
              <TextField size='small' variant='outlined' label='Total' value={bill.total} error={formErrors.total.invalid} onChange={inputChangeHandler} name='total' id='total' InputProps={{
                readOnly: true,
                startAdornment: <InputAdornment position='start'>&#8377;</InputAdornment>
              }} />
            ...
            <FormControl fullWidth error={formErrors.dueDate.invalid}>
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  renderInput={(params) => <TextField name='dueDate' size='small' {...params} error={formErrors.dueDate.invalid} />}
                />
              </LocalizationProvider>
              {formErrors.dueDate.invalid && <FormHelperText>{formErrors.dueDate.message}</FormHelperText>}
            </FormControl>
          </div>
          <div className='grid-item'>
            <FormControl fullWidth>
              <TextField size='small' variant='outlined' label='Remark' value={bill.remark} onChange={inputChangeHandler} name='remark' id='remark' />
            </FormControl>
          </div>
        </div>
      </form>
      <div className='right'>
        ...
        <Button variant='contained' size='medium' type='submit' color='primary' form='billForm' className='ml6'>Save</Button>
      </div>
    </Paper>
  </>;
};

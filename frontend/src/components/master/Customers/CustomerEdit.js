...
import { getBranches, getCustomer, updateCustomer } from '../../../lib/api-master';
...
const CustomerEdit = () => {
  ...
  useEffect(() => {
    ...
    if (isSubmitted && !hasErrors) {
      setIsLoading(true);
      updateCustomer(customer, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
          ...
          <div className='grid grid-6-col'>
            ...
            <div className='grid-item'>
              <FormControl fullWidth>
                <TextField size='small' variant='outlined' label='Name' value={customer.name} onChange={inputChangeHandler} name='name' id='name' />
              </FormControl>
            </div>
            <div className='grid-item'>
              <FormControl fullWidth>

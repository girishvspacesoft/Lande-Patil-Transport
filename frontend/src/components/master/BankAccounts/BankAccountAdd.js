...
import { addBankAccount, getBanks } from '../../../lib/api-master';
...
const BankAccountAdd = () => {
  ...
  useEffect(() => {
    ...
    if (isSubmitted && !hasErrors) {
      addBankAccount(bankAccount, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
  ...
  const validateForm = (formData) => {
    ...
    if (formData.openingBalance.trim() !== '' && isNaN(formData.openingBalance) && isNaN(parseFloat(formData.openingBalance))) {
    ...
    <Paper sx={{ padding: '20px', marginBottom: '20px' }}>
      <form action='' onSubmit={submitHandler}>
        <div className='grid grid-6-col'>
          ...
          <div className='grid-item'>
            <FormControl fullWidth>
              <TextField size='small' variant='outlined' label='Opening balance' value={bankAccount.openingBalance} onChange={inputChangeHandler} name='openingBalance' id='openingBalance' />
            </FormControl>
          </div>
        ...
        <div className='right'>
          ...
          <Button variant='outlined' size='medium' type='submit' color='primary' className='ml6'>Save</Button>
        </div>
      </form>
    </Paper>
  </>;

...
import { getVehicle, updateVehicle, getVehicleTypes, getSuppliers } from '../../../lib/api-master';
...
const VehicleAdd = () => {
  ...
  useEffect(() => {
    ...
    if (vehicleId) {
      setIsLoading(true);
      getVehicle(vehicleId, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
            setVehicle(response);
            setFetchedVehicle(response);
          }
          setIsLoading(false);
        })
        .catch(error => {
  ...
  useEffect(() => {
    ...
    if (isSubmitted && !hasErrors) {
      setIsLoading(true);
      updateVehicle(vehicle, controller)
        .then(response => {
          if (response.message) {
            setHttpError(response.message);
          } else {
